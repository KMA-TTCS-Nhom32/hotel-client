...
export const formatDate = (
  ...
  const getDayName = () => {
    ...
    return lng === 'vi' 
      ? DAY_NAMES[type].vi[dayjsDate.day()]
      : DAY_NAMES[type].en[dayjsDate.day()];
  };

  const getMonthName = () => {
    const type = isShortType ? 'short' : 'full';
    return lng === 'vi'
      ? MONTH_NAMES[type].vi[dayjsDate.month()]
      : dayjsDate.format('MMM');
  };

  const timeString = showTime ? `${dayjsDate.format('HH:mm')} - ` : '';
  const dateString = `${getDayName()}, ${dayjsDate.format('DD')} ${getMonthName()} ${dayjsDate.format('YYYY')}`;
